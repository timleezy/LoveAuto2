...
jQuery(document).ready( function( $) {
	...
	( function() {
		...
		if ( $( '#events-scroll' ).length ) {
			var Scrollbar = window.Scrollbar;
  			Scrollbar.init( document.querySelector('#events-scroll'), { alwaysShowTracks: true } );
  		}
		...
		function doParallax() {
			...
			for(var i in images) {
				...
				if ( $( el ).css( 'background-image' ) != 'none' ) {
					...
					el.style.backgroundPosition = '50% '+Math.round((elOffset - offset)*3/12)+'px';
				}
			}
		}
	...
	( function() {
		...
		if ( $( '#scamp_player' ).length <= 0 ) {
			return
		}

		settings.audio_player = new $.ScampPlayer( $( '#scamp_player' ), {

			/* Default Scamp Player options */
			volume : 70, /* Start volume level */
			autoplay : false, /* Autoplay track */
			no_track_image : 'js/audio-player/img/no-track-image.png', /* Placeholder image for track cover */
		...
		$( '#scamp_player' ).on( 'classChanged', function(){ 
			...
			if ( $( this ).hasClass( 'playing' ) ) {
				...
				p = pos/100;
				x = p * max;
				$( '.nav-player-btn.circle-btn .circle' ).css({ 'stroke-dasharray': + x + ' 144' });
		...
		if ( $container.length) {
			...
			var _items_filter = function( $el, $data) {
				...
				$el.on( 'click', 'a', function(e){
					...
					$( '.item-filter:not(.active-filter) li:first-child a' ).addClass( 'is-active' );
		...
		setTimeout( function(){
			$( '.site' ).imagesLoaded( { background: true } )
				.always( function( instance ) {
					...
					setTimeout( function(){
						$( '.loading-layer' ).css( 'visibility', 'hidden' ).removeClass( 'hide-layer show-layer' );
			  	...
			  	.progress( function( instance, image ) {
			  		...
			  		if ( image.isLoaded ) {
				    	...
				    	var 
				    		countLoadedImages = $( '.site img.loaded ').length,
